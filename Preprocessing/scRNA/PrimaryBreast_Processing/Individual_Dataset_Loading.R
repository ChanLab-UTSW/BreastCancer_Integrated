...
MetaDir <- "/project/InternalMedicine/Chan_lab/shared/IntegratedBreast_s204665/Preprocessing/scRNA"
OrigDataDir <- "/project/InternalMedicine/Chan_lab/shared/IntegratedBreast_s204665/Preprocessing/scRNA/PrimscRNA_Orig_Dataset_Files"
...
library(limma) #v3.50.1
library(org.Hs.eg.db) #v3.14.0
library(crayon) #v1.5.1
...
Convert_to_Current_Alias <- function(exp_mat, Human_or_Mouse = "Hs"){
  
  Human_or_Mouse <- match.arg(Human_or_Mouse, c("Hs", "Mm"))
  stopifnot("`exp_mat` must be a dgCMatrix or a dataframe. \nPlease convert using `exp_mat` <- as.data.frame(`exp_mat`) or `exp_mat` <- as.matrix(`exp_mat`, \"dgCMatrix\"). \nPress \"c\" and the enter key when done." = ((class(exp_mat) == "dgCMatrix") | is.data.frame(exp_mat)))
  
  if (Human_or_Mouse == "Hs"){
  ...
  library(limma)
  oldgenes <- rownames(exp_mat)
  newgenes <- alias2SymbolTable(oldgenes, species = Human_or_Mouse)
  usegenes <- ifelse(is.na(newgenes),oldgenes,newgenes)
  
  new_mat <- exp_mat[!duplicated(usegenes),]
  rownames(new_mat) <- unique(usegenes)
  new_mat <- new_mat[which(!(rownames(new_mat) %in%
                               usegenes[duplicated(usegenes)])),]
  
  
  if(class(new_mat) == "dgCMatrix"){
    for (i in unique(usegenes[duplicated(usegenes)])) {
      j <- exp_mat[which(usegenes == i),]
      j <- apply(j, 2, FUN = max)
      k <- as.matrix(t(j), "dgCMatrix")
      #k <- data.frame(t(j))
      rownames(k) <- i
...
Seurat_fromExpr_pdat <- function(joined_mat, joined_pdat, fdat.source,
                                 gene.col.fdat = 1, fdat_colnames = c("gene_short_name")){
  
  # _________________________________________________________________
  rownames(joined_pdat) <- joined_pdat$cells
  fdat <- as.matrix(fdat.source)
  #_________________________________________________________________
  rownames(fdat) <- fdat[,gene.col.fdat]
  fdat <- data.frame(fdat)
  colnames(fdat) <- fdat_colnames
  #_________________________________________________________________
  sobj.tmp <- CreateSeuratObject(counts = joined_mat)
  joined_pdat <- as.data.frame(joined_pdat)
  sobj.tmp <-AddMetaData(sobj.tmp,metadata=joined_pdat)
  cat(blue$bold("Metadata Preview\n"))
  out <- head(sobj.tmp@meta.data)
  print(out)
  #_________________________________________________________________
  sobj.tmp[["RNA"]]@meta.features<-fdat
  cat(blue$bold("Metafeatures Preview\n"))
  out <- head(sobj.tmp[["RNA"]]@meta.features)
  print(out)
  slotNames(sobj.tmp[["RNA"]])
...
setwd(MetaDir)
primMetaFull<-read_xlsx("Supplemental Tables Final.xlsx",
                        sheet="Table1_Dataset Information", skip = 1)
primMetaFull <- as.data.frame(primMetaFull)
primMetaFull[primMetaFull == "-"] <- NA

#Dataset Information
DatasetMeta <- primMetaFull[1:10,-c(18:24)]
colnames(DatasetMeta)
head(DatasetMeta)
DatasetMeta <- DatasetMeta[!(DatasetMeta$Dataset=="Bassez et al." | DatasetMeta$Dataset=="Total"),]
table(DatasetMeta$Dataset)
rownames(DatasetMeta) <- 1:dim(DatasetMeta)[1]

#Clinical Information
SampleMeta <- primMetaFull[12:dim(primMetaFull)[1],-24]
colnames(SampleMeta) <- SampleMeta[1,]
SampleMeta <- SampleMeta[-1,]
colnames(SampleMeta)
head(SampleMeta)
SampleMeta <- SampleMeta[!(SampleMeta$Dataset=="Bassez et al." | SampleMeta$Dataset=="Total"),]
table(SampleMeta$Dataset)
rownames(SampleMeta) <- 1:dim(SampleMeta)[1]
...
#load in datasets from NCBI GEO
GEOnums <- DatasetMeta$`scRNA-seq Data Access`[c(3,4,6, 8:9)]
#including the separate links for Azizi's two datasets 
# (in TableS1 the super series number GSE114727 is listed instead 
# for easier page access)
GEOnums <-  c("GSE114725", "GSE114724", GEOnums)

options(timeout=1000)
setwd("./IntegratedBreast_s204665/Dataset_Files/")
for (num in GEOnums){
  
  out <- paste0("Downloading data for GEO number ", num, "")  # Some output
  print(out)
  
  filePaths = getGEOSuppFiles(num) 
  
  out <- paste0("Finished downloading data for GEO number ", num, "")  # Some output
  print(out)
}

for (num in GEOnums){
  out <- paste0("Processing files for GEO number ", num, "")  # Some output
  print(out)
  
  tarOnly <- list.files(path = paste0("./", num, "/"), pattern = "*.tar$", full.names = TRUE) 
  gzOnly <- list.files(path = paste0("./", num, "/"), pattern = "*[^tar].gz$", full.names = TRUE) 
  targz <- list.files(path = paste0("./", num, "/"), pattern = "*.tar.gz$", full.names = TRUE) 
  
  #.tar.gz files
  if(!identical(targz, character(0))){
    gzipF <- list.files(path = paste0("./", num, "/"), pattern = "*.gz$", full.names = TRUE) 
    ldply(.data = gzipF, .fun = gunzip) 
    tarF <- list.files(path = paste0("./", num, "/"), pattern = "*.tar$", full.names = TRUE) 
    untar(tarF, exdir = paste0("./", num, "/")) 
  }
  
  #.tar files
  else if (!identical(tarOnly, character(0))){
    untar(tarOnly, exdir = paste0("./", num, "/"))
    gzipF <- list.files(path = paste0("./", num, "/"), pattern = "*.gz$", full.names = TRUE) 
    if (!identical(gzipF, character(0))){
    ldply(.data = gzipF, .fun = gunzip) 
    }

  }
  #.gz files
  else if (!identical(gzOnly, character(0))){
    ldply(.data = gzOnly, .fun = gunzip) 
  }
  
  out <- paste0("Files decompressed for GEO number ", num, "")  # Some output
  print(out)
}
...
list.files(path = "./GSE114725/", pattern = "*.csv$", full.names = TRUE)
...
list.files(path = "./GSE114724/", pattern = "*.csv$", full.names = TRUE) 
...
BC01BC08fullmatflip <- data.frame(t(BC01BC08fullmatold))
colnames(BC01BC08fullmatflip) <-paste(BC01BC08fullmatflip[1,], BC01BC08fullmatflip[5,], sep = "_")
colnames(BC01BC08fullmatflip) <- gsub(" ","",colnames(BC01BC08fullmatflip))

#meta
BC01BC08meta <- as.data.frame(t(BC01BC08fullmatflip[c(1:3),]))
identical(rownames(BC01BC08meta), colnames(BC01BC08fullmatflip)) #is true
colnames(BC01BC08meta) <- c("Patient", "Tissue.Source", "replicate")
#rownames(BC01BC08meta) <- gsub(" ","",rownames(BC01BC08meta))
BC01BC08meta[1:5,1:3]

BC01BC08fullmatflip <- BC01BC08fullmatflip[-c(1:5),]

BC01BC08fullmatflip[1:5,1:5]


#takes pdata information from original matrix
BC01BC08meta$cells <- colnames(BC01BC08fullmatflip)
BC01BC08meta[1:5,1:5]
BC01BC08meta$samples <- paste(BC01BC08meta$Patient, BC01BC08meta$Tissue.Source, sep = "_")
BC01BC08meta$samples <- paste(BC01BC08meta$samples, BC01BC08meta$replicate, sep = "_")
...
AziziSampleMeta <- SampleMeta[SampleMeta$Dataset == "Azizi et al.",]
colnames(AziziSampleMeta)[3] <- "Patient"
AziziSampleMeta_immune <- AziziSampleMeta[1:8,3:23]

BC01BC08meta <- merge(BC01BC08meta, AziziSampleMeta_immune, by = "Patient")

AziziDatasetMeta <- DatasetMeta[DatasetMeta$Dataset == "Azizi et al.",]
BC01BC08meta$Library.Preparation <- AziziDatasetMeta$`Library Preparation`
BC01BC08meta$Capture.Method <- AziziDatasetMeta$Method
BC01BC08meta$cDNA.Amplification <- AziziDatasetMeta$`cD- Amplification`
BC01BC08meta$Sequencer <- AziziDatasetMeta$Sequencer
BC01BC08meta$Gene.Coverage <- AziziDatasetMeta$`Gene Coverage`

rownames(BC01BC08meta) <- BC01BC08meta$cells
head(BC01BC08meta)

BC01BC08meta$Tissue.Source[which(BC01BC08meta$Tissue.Source == "BLOOD")] <- "Blood"
BC01BC08meta$Tissue.Source[which(BC01BC08meta$Tissue.Source == "NORMAL")] <- "Adjacent Normal Breast"
BC01BC08meta$Tissue.Source[which(BC01BC08meta$Tissue.Source == "TUMOR")] <- "Primary Tumor"

BC01BC08meta$RNA.Type <- "mRNA"

#convert gene name -
convertedmat <- Convert_to_Current_Alias(BC01BC08fullmatflip)

sobj <- Seurat_fromExpr_pdat(convertedmat, BC01BC08meta, rownames(BC01BC08fullmatflip))

azizi <- subset(test, subset = Original_Dataset == "Aziziimmune")

#changes flipped name to simpler name 
BC01BC08fullmat <- BC01BC08fullmatflip
BC01BC08fullmat[1:5,1:5]
rm(BC01BC08fullmatflip)

# patient.expmat.list <- list()
# for (patient in unique(BC01BC08fullmat_pdat$Patient))
# {
#   mat <- grep(pattern =c(paste0("^", patient)) , x = colnames(BC01BC08fullmat), value = TRUE)
#   mat = BC01BC08fullmat[,grepl(c(paste0("^", patient)),colnames(BC01BC08fullmat))]
#   mat[1:5,1:5]
#   patient.expmat.list[[patient]] <- mat
# }
# patient.expmat.list$BC1[1:5,1:5]
# 
...
BC9pdat_T1$Capture.Method <- "10X Genomics Chromium v2 5'"
...
BC9pdat_T2$Library.Preparation <- "10X Genomics Chromium v2 5'"
BC9pdat_T2$Capture.Method <- "10X Genomics Chromium v2 5'"
...
BC10pdat_T$Library.Preparation <- "10X Genomics Chromium v2 5'"
BC10pdat_T$Capture.Method <- "10X Genomics Chromium v2 5'"
...
BC11pdat_T1$Library.Preparation <- "10X Genomics Chromium v2 5'"
BC11pdat_T1$Capture.Method <- "10X Genomics Chromium v2 5'"
...
BC11pdat_T2$Library.Preparation <- "10X Genomics Chromium v2 5'"
BC11pdat_T2$Capture.Method <- "10X Genomics Chromium v2 5'"
...
gzKara <- list.files(path = "./GSE118389/", pattern = "*.gz$", full.names = TRUE) 
ldply(.data = gzKara, .fun = gunzip) 
...
setwd(OrigDataDir)
mat <- readMM(file = './PanCancer_Breastsamples/export/BC_counts/matrix.mtx')
mat <- as.matrix(mat)
mat[1:5,1:5]
dim(mat)

genes <- read.table(file = './PanCancer_Breastsamples/export/BC_counts/genes.tsv', 
                    sep = '\t', header = FALSE, stringsAsFactors = FALSE)
head(genes)
rownames(mat) <- genes[,1]
mat[1:5,1:5]

bar <- read.table(file = './PanCancer_Breastsamples/export/BC_counts/barcodes.tsv', 
                  sep = '\t', header = FALSE, stringsAsFactors = FALSE)
colnames(mat) <- bar[,1]
mat[1:5,1:5]
...
meta <- read.csv(file = './PanCancer_Breastsamples/2103-Breastcancer_metadata.csv')
head(meta)
...
pdat20_1 <- data.frame("samples" = colnames(matrix20_1), "Patient" = "TIL20", "Batch" = "20_1", "Molecular Subtype" = "TNBC",
                       "RNA Type" = "mRNA", "Gene Coverage" = "3'", "Library Preparation" = "10X Genomics Single Cell 3' v2", 
                       "Capture Method" = "10X Genomics Single Cell 3' v2",
...
sobj_pre$Treatment.Status[which(sobj_pre$Treatment.Status == "Patient 3")] <- "Naive"
